...
if (document.addEventListener) {
	document.addEventListener("mousedown", calendarCloseCheck, false);
} else if (document.attachEvent) {
	document.attachEvent("onmousedown", calendarCloseCheck);
} else {
	document["onmousedown"] = calendarCloseCheck;
}

function calendarCloseCheck(aEvent) {
	doc = document;
	var posx = 0;
	var posy = 0;
	...
	if (aEvent.pageX || aEvent.pageY) { // pageX/Y 표준 검사
		...
		posx = aEvent.clientX;
		posy = aEvent.clientY;
		if (navigator.userAgent.toLowerCase().indexOf("msie") != -1 && navigator.userAgent.toLowerCase().indexOf("opera") == -1) { // IE 여부 검사
			posx += doc.documentElement.scrollLeft;
			posy += doc.documentElement.scrollTop;
		}
	}

	if(posx < calendarDiv.offsetLeft || posx > (calendarDiv.offsetLeft + calendarDiv.offsetWidth)
		|| posy < calendarDiv.offsetTop || posy > (calendarDiv.offsetTop + calendarDiv.offsetHeight)) {
		closeCalendar();
	}
}
//end/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
var pathToImages = '/conts/images/calendar/';	// Relative to your HTML file
...
function writeTopBar()
{
	...
	leftDiv.className = 'leftArrow1';
	var img = document.createElement('IMG');
	img.src = pathToImages + 'left.gif';
	img.setAttribute("alt","이전년도");
	img.onmouseover = highlightArrow;
	img.onclick = switchYear;
	img.onmouseout = highlightArrow;
	img.style.width = '13px';
	img.style.height = '13px';
	leftDiv.appendChild(img);
	topBar.appendChild(leftDiv);
	if(Opera)leftDiv.style.width = '13px';
	...
	yearPicker.style.top = leftDiv.offsetTop + leftDiv.offsetHeight + 1 + 'px';
	...
	rightDiv.className = 'rightArrow1';
	var img = document.createElement('IMG');
	img.src = pathToImages + 'right.gif';
	img.setAttribute("alt","다음 년도");
	img.onclick = switchYear;
	img.onmouseover = highlightArrow;
	img.onmouseout = highlightArrow;
	img.style.width = '13px';
	...
	if(Opera)rightDiv.style.width = '13px';
	topBar.appendChild(rightDiv);
	
	// Left arrow
	var leftDiv = document.createElement('DIV');
	//leftDiv.style.marginRight = '1px';
	leftDiv.className = 'leftArrow2';
	var img = document.createElement('IMG');
	img.src = pathToImages + 'left.gif';
	img.setAttribute("alt","이전 월");
	img.onmouseover = highlightArrow;
	...
	var rightDiv = document.createElement('DIV');
	//rightDiv.style.marginRight = '1px';
	rightDiv.className = 'rightArrow2';
	var img = document.createElement('IMG');
	img.src = pathToImages + 'right.gif';
	img.setAttribute("alt","다음 월");
	img.onclick = switchMonth;
	img.onmouseover = highlightArrow;
	img.onmouseout = highlightArrow;
	img.style.width = '13px';
...
function writeCalendarContent()
{
	...
	calTable.setAttribute('summary','조회기간의 연도 및 월을 달력에서 선택하여 입력하는 표');
	calTable.width = '100%';
	calTable.cellSpacing = '0';
	calendarContentDiv.appendChild(calTable);	
	
	var caption = document.createElement('caption');
	caption.innerHTML = '달력';
	calTable.appendChild(caption);
   ...
   if (showWeekNumber) {
	...
	for(var no=0;no<dayArray.length;no++){
		...
		if ( no == 0) {
			cell.className = 'calendar_week_red';
		} else if ( no == 6) {
			cell.className = 'calendar_week_blue';
		} else {
			cell.className = 'calendar_week_cell';
		}
	}
   ...
   if (showWeekNumber) {
    ...
    if ( dayStartOfMonth < 6 ) {
		for(var no=0;no<dayStartOfMonth+1;no++){
			var cell = row.insertCell(-1);
			cell.innerHTML = '&nbsp;';
		}
    }

	var colCounter = dayStartOfMonth+1;
	...
	for(var no=1;no<=daysInMonth;no++){
		...
		if ( (colCounter)%7 == 0) {
			cell.className = 'calendar_cell_red';
		} else if ( (colCounter+1)%7 == 0) {
			cell.className = 'calendar_cell_blue';
		} else {
...
function initCalendar()
{
	if(MSIE){
		...
		// 수정 - IE6 layer poup에서의 select 태그문제 해결
		iframeObj.style.zIndex = 9999;
 ...
 * @author : 이은화
 */
function cfCheckTwoMonthsStr2(startDateStr, endDateStr, limit){
